...
Architecture:
  - Generator (Qwen):  generates math problems  → qwen_client
  - Verifier (DeepSeek): evaluates and scores     → deepseek_client

Splitting generation and verification across different models breaks the
fundamentally circular dependency inherent in self-verification.
"""
...
def generator(node: 'QuestionNode', new_skill: str, reference_examples: Optional[str] = None, 
    ...
    """
    print(f"\n[DEBUG] ===== generator 函数开始 (模型: {QWEN_MODEL}) =====")
    ...
    for retry_count in range(max_retries):
        # Call Qwen API
        try:
            print(f"[DEBUG] 开始调用 Qwen API... (尝试 {retry_count + 1}/{max_retries})")
            print(f"[DEBUG] 请求参数: model={QWEN_MODEL}, temperature=0.7, max_tokens=20000")
            ...
            print(f"[DEBUG] Qwen API 调用异常: {type(e).__name__}: {str(e)}")
            import traceback
...
def verifier(node: 'QuestionNode', reference_examples: Optional[str] = None, 
    ...
    Uses DeepSeek model via deepseek_client.
    ...
    """
    print(f"\n[DEBUG] ===== verifier 函数开始 (模型: {DEEPSEEK_MODEL}) =====")
    ...
    for retry_count in range(max_retries):
        # Call DeepSeek API
        try:
            print(f"[DEBUG] 开始调用 DeepSeek API... (尝试 {retry_count + 1}/{max_retries})")
            print(f"[DEBUG] 请求参数: model={DEEPSEEK_MODEL}, temperature=0.3, max_tokens=20000")
            ...
            print(f"[DEBUG] DeepSeek API 调用异常: {type(e).__name__}: {str(e)}")
            import traceback
            print(f"[DEBUG] 异常堆栈:\n{traceback.format_exc()}")
            if retry_count < max_retries - 1:
                wait_time = (retry_count + 1) * 2
...
def parse_generator_output(output: str) -> dict:
    ...
    # Extract 扣分点 (Deduction points) -> deduction_points
    match = re.search(r'###\s*扣分点\s*\n(.*?)(?=###|$)', output, re.DOTALL)
    if match:
        result['deduction_points'] = match.group(1).strip()
...
def optimizer(node: 'QuestionNode', deduction_points: str, max_retries: int = 3) -> str:
    ...
    """
    print(f"\n[DEBUG] ===== optimizer 函数开始 (模型: {DEEPSEEK_MODEL}) =====")
